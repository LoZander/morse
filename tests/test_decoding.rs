...
fn test_decode_sos() {
    ...
    let plaintext = morse::decode(ciphertext, &SymbolMap::default()).unwrap();
...
fn test_decode_invalid_morse_seq_gives_err() {
    let ciphertext = String::from(".......... ---");
    let res = morse::decode(ciphertext, &SymbolMap::default());
...
fn test_decode_invalid_morse_symbol_gives_err() {
    ...
    let res = morse::decode(cihpertext, &SymbolMap::default());
    let error = res.expect_err("expected error");
